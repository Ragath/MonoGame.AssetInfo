...
[ContentImporter(".info", DisplayName = "MGCB Importer", DefaultProcessor = nameof(MGCBProcessor))]
public class MGCBImporter : ContentImporter<AssetInfoContent[]>
{
    public override AssetInfoContent[] Import(string filename, ContentImporterContext context)
    {
        ...
        var files = matcher.GetResultsInFullPath(Path.GetDirectoryName(filename)).ToArray();

        foreach (var f in files)
            context.AddDependency(f);

        return files.SelectMany(file => ParseMGCB(File.ReadLines(file)))
                .ToArray();
